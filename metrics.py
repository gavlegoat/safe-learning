...
def distance_between_linear_function_and_neural_network(env, actor, K,
        terminal_err=0.01, rounds=10, steps=500):
...
def neural_network_performance(env, actor, terminal_err=0.01, rounds=10,
        steps=500):
...
def linear_function_performance(env, K, terminal_err=0.01, rounds=100,
        steps=500):
...
def find_boundary(x, x_max, x_min):
    ...
    max_update = (x > x_max)
    min_update = (x < x_min)
    x_max = np.multiply(x,max_update) + \
            np.multiply(x_max, np.logical_not(max_update))
    x_min = np.multiply(x,min_update) + \
            np.multiply(x_min, np.logical_not(min_update))
