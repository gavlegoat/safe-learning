...
def selfdrive(learning_method, number_of_rollouts, simulation_steps,
    ...
    def f(x, u):
      #We have two aeroplanes with 2 inputs for each controlling its own
      #angular velocity!
    ...
    def safety_reward(x, Q, u, R):
        return -np.matrix([[-(x[0,0] - cr) * (cl - x[0,0])]])
    ...
    def random_test(f, K, simulation_steps, continuous=True, timestep=h):
        ...
        for i in range(100):
            ...
            reward = test_controller_helper (f, K, x0, simulation_steps, testf,
                    continuous=True, timestep=h)
            if reward < 0:
    ...
    invs = [(np.matrix([[1, 0], [-1, 0], [0, 1], [0, -1]]),
            np.matrix([[1], [1], [pi/4], [pi/4]]))]
    covers = [(invs[0][0], invs[0][1], s_min, s_max)]
    initial_shield = Shield(env, K_list=Ks, inv_list=invs, cover_list=covers,
            bound=episode_len)

    actor, shield = DDPG(env, args=args, rewardf=safety_reward,
            safe_training=safe_training, shields=shields, penalty_ratio=penalty_ratio)
    ...
    #        simulation_steps, explore_mag = 0.04, step_size = 0.03,
...
if __name__ == "__main__":
    ...
    selfdrive("random_search", 200, 200, eps, [300, 200], [300, 250, 200],
