...
def get_disjointness_constraint(x_dim, P1, P2, str_identifier):
    ...
    #Using Farkas Lemma, we instead have to assert that there is a y for which
    #  E^T y = 0, f^T y > 0 (and not f^T y < 0, notice how we represent
    #  polytopes) and y >= 0
    ...
    for i in range(x_dim):
        lst = [A[j][i]*y[j] for j in range(m1)] + \
                [C[j][i]*y[j+m1] for j in range(m2)]
        list_of_constraints.append(Sum(lst) == 0)

    #Next assert f^T > 0
    lst = [b[j] * y[j] for j in range(m1)] + \
            [d[j] * y[j+m1] for j in range(m2)]
    list_of_constraints.append(Sum(lst) > 0)
...
def get_disjointness_constraint_rect(x_dim, rect_1, rect_2):
    ...
    for i in range(x_dim):
        ...
        lo_i_2 = rect_2[i][0]
        hi_i_2 = rect_2[i][1]
        if lo_i_2 is not None:
            list_of_constraints.append(hi_i_1 < lo_i_2)
        if hi_i_2 is not None:
            list_of_constraints.append(lo_i_1 > hi_i_2)

    return Or(list_of_constraints)

def get_rect_poly_containment_constraint(rect, poly):
    #rect = [(lo_1, hi_1), (lo_2, hi_2), ..., (lo_k, hi_k)] represents a
    #  polytope, with k = dimension of the state space.
    #target_poly = (target_mat, target_vec) is the target polyhedron
    #We will check if each of the vertices of the reach set is inside the
    #  target polyhedron
    ...
    for corner in corner_list:
        list_of_constraints.append(
                get_point_polytope_membership_constraint(corner, poly))
    return And(list_of_constraints)

def get_rect_rect_containment_constraint(rect1, rect2):
    #rect1 and rect2 = [(lo_1, hi_1), (lo_2, hi_2), ..., (lo_k, hi_k)]
    #  represents a polytope, with k = dimension of the state space.
...
def get_avoid_constraint_rect(interval_list, avoid_list):
    #interval_list = [(lo_1, hi_1), (lo_2, hi_2), ..., (lo_k, hi_k)] represents
    #a polytope, with k = dimension of the state space.
    ...
    for avoid in avoid_list:
        ...
        if avoid_flag:
            list_of_constraints.append(
                    get_disjointness_constraint_rect(
                        dim, interval_list, avoid_desc))
        else:
            list_of_constraints.append(
                    get_disjointness_constraint_poly_incomplete(
                        dim, corner_list, avoid_desc))
    return And(list_of_constraints)
...
def add_constraints(s, x, Theta, x_dim, A, B, K, target, avoid_list,
        avoid_list_dynamic, num_steps):
    #s is a z3 Solver
    #x, u and r are z3 variables
    #Theta, is the inital set
    #radius_list = [lst_1, lst_2, ..., lst_m], where m= num_steps,
    #  and each lst_i = [r1, ..., r_k], k = x_dim is the per-dimension constant
    #  factor to be multiplied at the i'th step
    ...
    #u_poly = (u_mat, u_vec) represents the space of inputs. That is, we allow
    #  any u that satisfies u_mat\times u + u_vec <= 0
    #target_poly = (target_mat, target_vec) is the target polytope
    #avoid_list is a list [(flag1, poly1), (flag2, poly2) ...  (flagk, polyk)],
    #  where flagi = True means rectangle, o/w general polytope. if
    #  flagi = True, thn polyi = list of x_dim pairs (hi, low). O/w it is
    #  (Ai, bi).

    is_dynamic_list_None = (avoid_list_dynamic is None)

    s.push()
    s.add(get_avoid_constraint(x[0], avoid_list if is_dynamic_list_None \
            else avoid_list + avoid_list_dynamic[0]))
    res = s.check()
    if res == sat:
    ...
    for i in range(num_steps):
        ...
        next_x_constraint = get_next_constraint_point(
                x[i], A, B, K, x_dim, x[i+1])
        safety_constraint = get_avoid_constraint(x[i+1],
                avoid_list if is_dynamic_list_None \
                        else avoid_list + avoid_list_dynamic[i+1])
...
def add_constraints_safety(s, x, Theta, x_dim, A, B, K, target, safe, num_steps):
    ...
    #x, u and r are z3 variables
    #Theta, is the inital set
    #radius_list = [lst_1, lst_2, ..., lst_m], where m= num_steps, and each
    #  lst_i = [r1, ..., r_k], k = x_dim is the per-dimension constant factor
    #  to be multiplied at the i'th step
    #A is a constant square matrix of dimension x_dim \times x_dim
    #u_dim is the dimension of the input space
    #B is the feedback matrix of dimension x_dim\times u_dim
    #u_poly = (u_mat, u_vec) represents the space of inputs. That is, we allow
...
def get_counterexample(s, x, num_steps):
    ...
    m = s.model()
    # print("model=", m)
    trajectory = [[m[x[i][j]].numerator_as_long() * 1.0 / \
            m[x[i][j]].denominator_as_long() for j in range(x_dim)] \
...
def check_covered(dim, set1, list_of_covers, useInv):
    ...
    if not useInv:
        for cover in list_of_covers:
            list_of_constraints.append(
                    Not(get_point_membership_constraint(x, cover)))
    ...
    if chk == sat:
        ...
        m = s_cover.model()
        # print("model=", m)
        return np.matrix([[m[x[j]].numerator_as_long() * 1.0 / \
                m[x[j]].denominator_as_long()] for j in range(dim)])
    elif chk == unsat:
        return None
    else:
        #Some problem
...
def bounded_z3(x0, initial_size, Theta, K, A, B, target, safe, avoid_list,
        avoid_list_dynamic, num_steps):
    ...
    x = [[Real("x_ref_%s[%s]" % (i, j+1)) for j in range(x_dim)] \
            for i in range(num_steps+1)]
    ...
    initial_rectangle = (True, [(x0[i,0] - initial_size[i],
                                 x0[i,0] + initial_size[i]) \
                                         for i in range(x_dim)])
    ...
    s_check = None
    if safe is None:
        s_check = add_constraints(s, x, Theta, x_dim, A, B, K, target,
                avoid_list, avoid_list_dynamic, num_steps)
    else:
        s_check = add_constraints_safety(s, x, Theta, x_dim, A, B, K, target,
                safe, num_steps)
...
def verify_controller_z3(x0, Theta, verification_oracle, learning_oracle,
        draw_oracle, continuous):
    if continuous:
        x_dim = len(x0)
        ...
        draw = True
        initial_size = np.array([max(s_max[i,0] - x0[i, 0],
            x0[i, 0] - s_min[i,0]) for i in range(x_dim)])
        ...
        while (not Theta_has_been_covered) and \
                number_of_steps_initial_size_has_been_halved < \
            ...
            while draw:
                ...
                if draw_oracle is not None:
                    ...
                    print("If the the following trjectory score is {} >= 0, then safe!".format(test_result))
                    if test_result >= 0:
            ...
            if number_of_steps_initial_size_has_been_halved == 0 and vresult:
                ...
                return True, resultList
                    ...
                    (1 - multiplicative_factor_for_radius)).any()) and \
                    (not useInv or not vresult):
                # A counterexample is found
                ...
                if number_of_steps_initial_size_has_been_halved == 0:
                    ...
                    h = (l + h) / 2
                    initial_size = (l + h) / 2
                number_of_steps_initial_size_has_been_halved += 1
                ...
                if pvresult is None or pvinv is None or pinitial_size is None:
                    ...
                    if not useInv:
                        ...
                        if type(initial_size) is np.ndarray:
                            for i in range(len(x0)):
                                x_i = x0[i,0]
                                cover.append((x_i-initial_size[i],
                                    x_i+initial_size[i]))
                        else:
                            for i in range(len(x0)):
                                x_i = x0[i,0]
                                cover.append((x_i-initial_size,
                                    x_i+initial_size))
                    ...
                    Theta_has_been_covered = x0 is None or \
                            number_of_steps_initial_size_has_been_halved == 0
                    number_of_steps_initial_size_has_been_halved = 0
